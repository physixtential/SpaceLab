...
def run_job(location,num_balls):
	cmd = ["python3", "{}run_sim_COPY.py".format(location), location, str(num_balls)]
	# print(cmd)
	subprocess.run(cmd)
...
if __name__ == '__main__':
	...
	try:
		...
		subprocess.run(["make","-C","ColliderSingleCore","ColliderSingleCore_COPY.x"], check=True)
	except:
		print('compilation failed')
		exit(-1)
		
	job_set_name = "singleCoreComparison_COPY"
	...
	for attempt in attempts:
		for n in N:
			for Temp in Temps:
				...
				input_json['note'] = "How far does this go?"
				####################################
				...
				os.system("cp default_files/run_sim_COPY.py {}run_sim_COPY.py".format(job))
				os.system("cp ColliderSingleCore/ColliderSingleCore_COPY.x {}ColliderSingleCore_COPY.x".format(job))
				folders.append(job)
	# print(folders)
