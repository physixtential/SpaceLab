...
def main():
	...
	ideal = [b for i in threads]
	# ideal = b/threads
	print(ideal)
	# ideal[0:2] = np.nan

	efficiency = (speedups[0,2])/(speedups[0,0]/threads[0])
	print(efficiency)
	...
	for f_i,folder in enumerate(folders):
		...
		ax.bar(threads[0],speedups[0,0],label='Weak scaling',color='g',width=1)
		ax.bar(threads[1],speedups[0,1],color='g',width=4)
		ax.bar(threads[2],speedups[0,2],color='g',width=16)
		# ax.plot(threads,speedups[0,:],label='Weak scaling',marker='.')
		ax.plot(threads,ideal,label="Ideal weak scaling",linewidth=2)
		ax.set_ylim((1,2**12))
		...
		y_major_ticks = [2**i for i in range(int(np.log2(1)), int(np.log2(np.max(speedups[0:]))) + 1)]
