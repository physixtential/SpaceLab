// There are 4 important steps to creating a new cluster:
// 1- allocateClusterMem(size)
// 2- fillCluster() sets ball positions, radii, masses, etc.
// 3- initPhysics() to set correct first step physics
...
struct cluster
{
	...
	void allocateClusterMem(int nBalls)
	{
		cNumBalls = nBalls;
	...
	void fillCluster(const double ballR, double range)
	{
	...
	void initPhysics(int cNumBalls)
	{
		mTotal = 0;
		KE = 0;
		PE = 0;
