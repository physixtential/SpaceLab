...
void lass::tofu()
{
	...
	for (int k = 0; k < outerLoop; k++)
	{
        ...
        for (pc = world_rank + 1; pc <= num_pairs; pc += world_size)
        {
            ...
            if (true) {
                ...
                if (write_step) {
                    ...
                    PE += 1.0;//U_vdw + 0.5 * k * overlap * overlap; ///TURN ON FOR REAL SIM
                }
            } else  // Non-contact forces:
            {
                ...
                if (write_step) {
                    ...
                    PE += 1.0;//U_vdw;  // Van Der Waals TURN ON FOR REAL SIM
        ...
        // #pragma acc update self(acc[0:particles],aacc[0:particles],PE) //if(write_step)
		    ...
		    // #pragma acc update host(acc[0:particles],aacc[0:particles],PE) //if(write_step)        
    }
