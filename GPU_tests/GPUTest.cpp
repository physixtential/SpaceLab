...
int main()
{
	...
	for (int k = 0; k < outerLoop; k++)
	{
	    ...
	    #pragma omp target map(tofrom:accum[:num_pairs])
	    {
	    ...
	    #pragma omp parallel for collapse(2) shared(accum) reduction(+:arrx[:PSEUDOPARTICLES],arry[:PSEUDOPARTICLES],arrz[:PSEUDOPARTICLES]) 
