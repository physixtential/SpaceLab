...
int main()
{
	...
	for (int k = 0; k < outerLoop; k++)
	{
	    ...
	    {
	    	#pragma omp parallel for default(none) reduction(+:arrx[:PSEUDOPARTICLES],arry[:PSEUDOPARTICLES],arrz[:PSEUDOPARTICLES]) shared(world_rank,world_size,num_pairs) private(i,j,pc)
	...
	if (world_rank == 0)
	{
		...
		for (int e = 0; e < PSEUDOPARTICLES; ++e)
		{
			...
			if (arrx[e] != arr_comp[e][0] || arry[e] != arr_comp[e][1] || arrz[e] != arr_comp[e][2])
			{
				count++;
