...
public:
    ...
    void sim_one_step(const bool write_step,timey t);
...
void Ball_group::sim_one_step(const bool write_step,timey t)
{
    ...
    #pragma omp parallel for default(none) reduction(vec3_sum:acc[:num_particles],aacc[:num_particles]) private(A,B) shared(num_particles,R,pos,vel,distances,w,moi,u_r,u_s,kout,kin,h_min,Ha,write_step)
    for (pc = 1; pc <= (((num_particles*num_particles)-num_particles)/2); pc++)
    {
