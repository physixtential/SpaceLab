...
//     int A,B;
//     #pragma omp parallel for collapse(1) default(none) reduction(+:O.acc[:O.num_particles],O.aacc[:O.num_particles]) private(A,B) shared(O.num_particles,O.r,O.pos,O.vel,O.distances,O.w,O.moi,u_r,u_s,kout,kin,h_min,Ha,write_step)
//     for (A = 1; A < O.num_particles; A++)  
//     {
