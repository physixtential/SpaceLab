...
BmpToTexture(char* filename, int* width, int* height)
{
	...
	if (VERBOSE)	fprintf(stderr, "Old oldNumExtra padding = %d\n", oldNumExtra);
...
ReadInt(FILE* fp)
{
	unsigned char b0 = fgetc(fp);
	unsigned char b1 = fgetc(fp);
	unsigned char b2 = fgetc(fp);
	unsigned char b3 = fgetc(fp);
