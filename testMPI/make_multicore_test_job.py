	...
	for attempt in attempts:
		for n in N:
			for thread in threads:
				...
				input_json['N'] = n
				input_json['simTimeSeconds'] = 1.0e-4 #Shorter sim time. Don't need whole time
				...
				input_json['note'] = "making 1200 particle agg, {} thread job.".format(thread)
				####################################
				...
				sbatchfile += "#SBATCH -N 5\n"
				# sbatchfile += "#SBATCH -c 32\n\n"
				sbatchfile += 'export OMP_NUM_THREADS={}\n'.format(thread)
				sbatchfile += 'export SLURM_CPU_BIND="cores"\n'
				sbatchfile += "srun -n 5 -c {} --cpu-bind=cores numactl --interleave=all ./ColliderMultiCore.x {} 2>sim_err.log 1>sim_out.log".format(thread*2,job)
