...
int main(int argc, char const* argv[])
{
	...
	for (int Step = 1; Step < steps; Step++) // Steps start at 1 because the 0 step is initial conditions.
	{
		...
		{
			float eta = (time(NULL) - startProgress) / 1000.0 * (steps - Step); // In seconds.
			//std::cout << "Average step time for last 1000 steps = " << eta << std::endl;
			std::cout << "Step: " << Step << " | Progress: " << (int)((double)Step / (double)steps * 100) << "% | Estimated time remaining: " << eta / 60.0 / 60.0 << " hours.           \r";
